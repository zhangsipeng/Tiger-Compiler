...
extern uint64_t GLOBAL_REC_DEC;
...
constexpr long END_MARK = 0;
  class Seg{
       ...
       bool operator ==(Seg tmp){
         return (start==tmp.start)&&(size==tmp.size);
       }  
    };

class TigerHeap {
public:
  // Todo(lab7): You need to implement those interface correctly accroding to your design.
