...
{
    for (auto p = liveness->GetLiveGraph().moves->GetList().begin(); p!=liveness->GetLiveGraph().moves->GetList().end(); p++) {
  ...
  for (auto p = liveness->GetLiveGraph().interf_graph->Nodes()->GetList().begin(); p!=liveness->GetLiveGraph().interf_graph->Nodes()->GetList().end(); p++) {
  ...
  auto registers = reg_manager->Registers_rsp();
  auto colors = reg_manager->colors;
  for (int i = 0; i < 15; i++) {
    std::string *color = new std::string;
    (*color) = (*colors)[i];
    coloring->Enter(registers->NthTemp(i), color);
  }
...
{
    return ((workListMoves->Union(activeMoves))->Intersect(moveList[node]))->GetList().size()!=0 ;
}
void RegAllocator::MakeWorklist()
{
 ...
 graph::NodeList<temp::Temp> * RegAllocator::Adjcent(graph::Node<temp::Temp> * node)
{
...
void RegAllocator::Simplify()
{
    ...
    auto ls=Adjcent(tmpNode)->GetList();
...
void RegAllocator::Coalesce()
{
  ...
  x = GetAlias(x);
  y = GetAlias(y);
  //precolored必须放第一个，后面总是把u放入worklist
  ...
  if (u == v) {
    ...
    AddWorkList(u);
    AddWorkList(v);
  } else if ((precolored(u->NodeInfo()) && All_OK(Adjcent(v), u))
    || (!precolored(u->NodeInfo()) && Conservative(Adjcent(u)->CatList(Adjcent(v))))){
    coalescedMoves->Append(m.first,m.second);
    Combine(u, v);
    AddWorkList(u);
  } else {
   ...
   while (!selectStack.empty()) {
    ...
    for (auto w : adjList[n]) {
      if (coloredNodes.find(GetAlias(w)) != coloredNodes.end() || precolored(GetAlias(w)->NodeInfo())) {
        // if (coloring->Look(GetAlias(w)->NodeInfo())==nullptr){
        //   printf("node info:%d err!\n",w->NodeInfo()->Int());
        // }
  ...
  for (auto node : spilledNodes) {
    ...
    for (int i=0;i<GC_Roots->size();i++){
      for (auto por=(*GC_Roots)[i].tmpArr.begin();por!=(*GC_Roots)[i].tmpArr.end();por++){
        ...
        if ((*por)==spilledTemp){
          (*GC_Roots)[i].tmpArr.erase(por);
          (*GC_Roots)[i].stackPtr.push_back(f->offset);
          break;
        }
      }
    }
    ...
    for (auto p = il->GetList().begin(); p!=il->GetList().end(); p++) {
      ...
      if (src && src->contains(spilledTemp) && dst && dst->contains(spilledTemp)) {
        temp::Temp *newTemp = temp::TempFactory::NewTemp(is_ptr);
        noSpillTemp.insert(newTemp);
        src->replace(spilledTemp, newTemp);
        dst->replace(spilledTemp, newTemp);
        std::string assem = "movq (" + f->name_->Name() + "_framesize" + std::to_string(f->offset) + ")(`s0), `d0";
        newIl->Append(new assem::OperInstr(assem, new temp::TempList(newTemp), new temp::TempList(reg_manager->StackPointer()), nullptr));
        newIl->Append((*p));
        assem = "movq `s0, (" + f->name_->Name() + "_framesize" + std::to_string(f->offset) + ")(`s1)";
        temp::TempList * args=new temp::TempList();
        args->Append(newTemp);
        args->Append(reg_manager->StackPointer());
        newIl->Append(new assem::OperInstr(assem, nullptr, args, nullptr));
        ...
        dst->replace(spilledTemp, newTemp);
        newIl->Append((*p));
        std::string assem = "movq `s0, (" + f->name_->Name() + "_framesize" + std::to_string(f->offset) + ")(`s1)";
        temp::TempList * args=new temp::TempList();
        args->Append(newTemp);
        args->Append(reg_manager->StackPointer());
        newIl->Append(new assem::OperInstr(assem, nullptr, args, nullptr));
...
void RegAllocator::RegAlloc(){
    ...
    cfg->AssemFlowGraph();
  //   cfg->GetFlowGraph()->Show(stderr,cfg->GetFlowGraph()->Nodes(),[co](assem::Instr * ir) {
  //   ir->Print(co);
  // });
    liveness=new live::LiveGraphFactory(cfg->GetFlowGraph(),frame_);
    liveness->Liveness();
  //   printf("liveness done!\n");
  //   liveness->GetLiveGraph().interf_graph->Show(stdout,liveness->GetLiveGraph().interf_graph->Nodes(),[](temp::Temp * t) {
  //   printf("nodeinfo:%d\n",t->Int());
  // });
    Build();
    printf("finish build\n");
  ...
  } while (!simplifyWorkList.empty() || workListMoves->GetList().size() || !freezeWorkList.empty() || !spillWorkList.empty());
