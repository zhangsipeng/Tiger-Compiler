...
namespace assem {
   class Gc_Root{
    ...
    std::string name;
    std::string retaddr;
    std::string callerstacksize;
    std::string stacksize;
    std::vector<int> stackPtr;
    std::vector<int> RegPtr;
    std::vector<temp::Temp *> tmpArr;
    int idx;
    Gc_Root(){
      RegPtr.resize(6);
      for (int i=0;i<6;i++){
...
public:
  ...
  virtual void Print(temp::Map *m) const = 0;
...
public:
  ...
  void Print( temp::Map *m) const override;
